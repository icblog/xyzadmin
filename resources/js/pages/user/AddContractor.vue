<template>
  <Layout pageTitle="add-regular-contractor" pageIntro="Add regular contractor">
            ...
            <div class="form-wrapper">
              ...
              <section v-if="!registerFormData.isRegisterFormProcessing">
                ...
                <div v-else-if="registerFormData.customSuccess != ''">
                  ...
                  <AfterSuccessBtn
                    add_link_name="/user/add-contractor"
                    redirect_link_name="/view-users"
                    click_action_add="add_another"
                    click_action_redirect="view_users"
                    add_btn_name="Add another contractor"
                    redirect_btn_name="View userss"
                ...
                <form v-else @submit.prevent="handleRegisterForm">
                  <div class="row">
                    ...
                    <div class="col-md-6">
                      <div class="form-group">
                        <OptionElementInput
                          v-if="companyRes.length > 0"
                          whatToget="id"
                          labelName="Company*"
                          :inputErrMsg="registerFormData.errors.company_err"
                          :defaultOptionSelected="
                            registerFormData.optionSelectedNameValue
                          "
                          :disabledInput="true"
                          :resultObj="companyRes"
                          :inputFocusFunc="() => removeRegisterFormError('company')"
                          @updateSelectedOptionInput="updateSelectedCompany"
                        />
                      </div>
                      <!-- end col-md-6 -->
                    </div>
                    <div class="col-md-6">
                      <div class="form-group">
                        ...
                        <div
                          class="text-danger small"
                          v-show="registerFormData.errors.badge_id_err != ''"
                        >
                          {{ registerFormData.errors.badge_id_err }}
                        </div>
                        <input
                          v-model="registerFormData.formData.badge_id"
                          type="text"
                          class="form-control"
                          maxlength="3"
                          autocomplete="off"
                          @focus="() => removeRegisterFormError('badge_id')"
                        />
                      </div>
                      <!-- end col-md-6 -->
                    </div>
...
const updateSelectedCompany = (selectedCompany) => {
  registerFormData.formData.selectedCompanyId = selectedCompany.id;
  registerFormData.optionSelectedNameValue = selectedCompany.name;
};
...
const removeRegisterFormError = (focusedInput) => {
  ...
  if (focusedInput == "company") {
    if (registerFormData.errors.company_err != "") {
      registerFormData.errors.company_err = "";
    }
  }

  if (focusedInput == "badge_id") {
    if (registerFormData.errors.badge_id_err != "") {
      registerFormData.errors.badge_id_err = "";
    }
  }
...
const assignRegisterFormErrorMsg = (errorsArr) => {
  ...
  errorsArr.forEach((error) => {
    ...
    if (error.toLowerCase().indexOf("company") > -1) {
      registerFormData.errors.company_err = error;
    }
    if (error.toLowerCase().indexOf("badge") > -1) {
      registerFormData.errors.badge_id_err = error;
    }
...
const checkForRegisterFormErrors = () => {
  ...
  if (registerFormData.formData.selectedCompanyId === "") {
    abort = true;
    errorsArr.push("Please select a company");
  }

  if (registerFormData.formData.badge_id === "") {
    abort = true;
    errorsArr.push("Please add a badge id");
  }
    ...
    try {
      ...
      if (res?.data?.errors.length > 0) {
        ...
        registerFormData.customSuccess =
          "Thanks, regular contractor has been added successfully.";
        setTimeout(() => {
