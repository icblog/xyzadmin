                  ...
                  <div v-else>
                    ...
                    >
                      ...
                      >
                        ...
                        <!-- EDIT QUICK VISITING REASON -->
                        <div v-show="returnIfInputIsActive(reason.id)" class="row">
                          <div class="col-md-12 mx-auto">
                            <div class="bg-white p-2 mr-5">
                              <label class="form-label mr-2"
                                >Make it a quick reason?*</label
                              >
                              <div class="form-check-inline">
                                <span class="form-check-label">
                                  <input
                                    ...
                                    value="no"
                                    :checked="reason.quick_reason == 'no' ? true : false"
                                    v-model="reactiveData.is_quick_reason"
                                  />No
                                </span>
                              </div>
                              <div class="form-check-inline">
                                <span class="form-check-label">
                                  <input
                                    ...
                                    class="form-check-input"
                                    value="yes"
                                    :checked="reason.quick_reason == 'yes' ? true : false"
                                    v-model="reactiveData.is_quick_reason"
                                  />Yes
                                </span>
                              </div>
...
import VisitingReasonsForm from "./VisitingReasonsForm";
...
const makeApiCall = async () => {
  router.post("/visiting-reasons", returnDataToBePost(), {
    onFinish: () => {
      if (reactiveData.is_searching) {
...
const handleDataUpdate = (index, data_id) => {
  ...
  if (
    reactiveData.data_old_value == reactiveData.data_new_value &&
    reactiveData.is_quick_reason == reactiveData.is_quick_reason_old
  ) {
    abort = true;
  ...
  if (!abort) {
    ...
    setTimeout(async () => {
      try {
        const res = await axios.post("/visitor/update-reasons", {
          data_id: data_id,
          data_new_value: reactiveData.data_new_value,
          data_old_value: reactiveData.data_old_value,
          is_quick_reason: reactiveData.is_quick_reason,
          is_quick_reason_old: reactiveData.is_quick_reason_old,
        });
        if (res.data?.error != "") {
          ...
          setTimeout(() => {
            ...
            reactiveData.api_res[index].quick_reason = reactiveData.is_quick_reason;
            reactiveData.edit_delete_success = "";
            reactiveData.data_being_edited_id = 0;
            reactiveData.show_ok_cancel_btn = false;
          }, 300);
...
const handleDeleteDataYes = (data_id, data_name) => {
  ...
  if (window.confirm(`Delete ${data_name}? please note this can not be undone.`)) {
    ...
    setTimeout(async () => {
      try {
        const res = await axios.post("/visitor/delete-reasons", {
          data_id: data_id,
        });
